          ...
          >
            <div className="flex items-center justify-between border-b border-white/70 bg-white/[0.03] px-6 py-5 backdrop-blur-md">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-serif text-2xl text-white">Saved Treasures</h3>
            ...
            <div className="flex-1 overflow-y-auto p-5 sm:p-6">
              {wishlistedProducts.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center rounded-3xl border border-dashed border-cocoa/15 bg-white/[0.03] p-8 text-center">
                  ...
                  <h4 className="mt-4 text-xl font-bold text-white">No saved items yet</h4>
                ...
                <div className="space-y-4">
                  {wishlistedProducts.map((product) => (
                    <article
                      key={product.id}
                      className="group overflow-hidden rounded-2xl border border-emerald-500/20 bg-white/[0.05] p-3.5 shadow-soft transition duration-300 hover:shadow-lift"
                    >
                        ...
                        <div className="flex min-w-0 flex-1 flex-col justify-between">
                          <div>
                            <div className="flex items-start justify-between gap-2">
                              <h5 className="truncate font-serif text-lg font-bold text-white">{product.title}</h5>
                              <button
                                type="button"
                          ...
                          <div className="mt-2 flex items-center justify-between gap-3">
                            <span className="text-base font-bold text-amber-400 font-extrabold">{product.price}</span>
                            <button
                              type="button"
                              onClick={() => {
                                addToCart(product, 1)
